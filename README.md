# Hi there! I'm Franco

Currently studying Systems Engineering at the [National University of Technology](https://frba.utn.edu.ar/) in Buenos Aires Argentina. Also a Teaching Assistant on Numerical Analysis at UTN.BA.

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=FrancoGiachetta&show_icons=true&theme=gruvbox" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=FrancoGiachetta&layout=compact&theme=gruvbox" />
</p>

## Work

- [cairo_native](https://github.com/starkware-libs/cairo_native): a Cairo's Sierra to MLIR compiler, made to speedup contracts at Starknet. *LambdaClass, 2024-2026*.
- [cairo_vm](https://github.com/starkware-libs/cairo-vm): a Cairo's ZkVM. *LambdaClass, 2024-2026*.

## Personal Projects

- [mathic](https://github.com/FrancoGiachetta/mathic): a programming language with builtin symbolic algebra capabilities.
